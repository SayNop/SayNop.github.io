...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div>`,7),u=e("li",null,[i("R10 加载内存偏移表中的机器码（偏移量） "),e("em",null,"10980")],-1),b=e("em",null,"off_680",-1),R=e("em",null,"680",-1),D=e("strong",null,"此处不涉及PC寄存器，因此最后不需要额外+4，详见参数文章链接",-1),p={href:"https://bbs.kanxue.com/thread-268869.htm",target:"_blank",rel:"noopener noreferrer"},C=e("li",null,[i("ADD执行完成后 "),e("em",null,"R10 = 10980 + 680 = 11000"),i(" ，即R10存放全局偏移表的起始地址")],-1),_=l(`<p><strong>补充</strong>：如何修改需要读取的字符串，请查看后续文章中的内容</p><p><strong>例3</strong>：以C++程序helloworld代码deregister_tm_clones函数为例说明如何通过 <strong>伪指令<code>LDR</code>载入全局偏移表（.got）内的数据</strong></p><div class="macos language-assembly line-numbers-mode" data-ext="assembly"><pre class="language-assembly"><code>.text:000006B4 deregister_tm_clones                    ; CODE XREF: __do_global_dtors_aux:loc_732↓p
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><ul><li>说明 <ul><li><p>内存偏移表中 对应项的被减数为对应项地址，该地址被r1加载（偏移已被adr内部处理）</p></li><li><p>在上一条的条件下，执行ldr加载内存偏移表并add，则会指向目标地址（已在例2中说明）</p></li></ul></li></ul><p>通过将偏移量转换成基本数据，得到如下等效代码（省略部分）</p><div class="language-assembly line-numbers-mode" data-ext="assembly"><pre class="language-assembly"><code>;             内存偏移表转换成具体数值 0x1468EA + 0xA - 0x145C4C = 0xCA8
...
LOAD:00145C78                 LDR             R2, =dword_4B4BC
LOAD:00145C7C                 ADD             R1, R1, R2
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="三级流水线" tabindex="-1"><a class="header-anchor" href="#三级流水线" aria-hidden="true">#</a> 三级流水线</h2><p>ARM7TDM使用三级流水线执行指令。</p><ol><li>内存中取回指令</li><li>开始解码</li><li>实际执行指令</li></ol><p>程序计数器（PC）总是<strong>超出当前执行指令的两条指令</strong> 。即第x条指令内的pc的值，实际上是第(x+2)条指令的地址</p><p>即在 <em>IDA</em> 动态调试时，若F7单步执行到某一条指令时，其实CPU已经开始处理后目标指令的两条指令了（下一条指令已开始解码，下下条指令已开始读取）。</p><p>故在调试过程中需要修改指令时，需在 <strong>执行前两条之前进行修改</strong> （执行到的指令与要修改的指令至少间隔2条指令）。</p><h2 id="so内调用函数的入参分析" tabindex="-1"><a class="header-anchor" href="#so内调用函数的入参分析" aria-hidden="true">#</a> so内调用函数的入参分析</h2><p>BL与BLX指令常常伴随函数调用出现。函数参数的传递是通过寄存器与堆栈实现的。</p><h3 id="通过寄存器确定函数调用" tabindex="-1"><a class="header-anchor" href="#通过寄存器确定函数调用" aria-hidden="true">#</a> 通过寄存器确定函数调用</h3><p>在寄存器列表中：</p><ul><li><em>R0 ~ R3</em> 共4个寄存器用于函数参数及返回值的传递（<strong>函数相关</strong>）</li><li><em>R4 ~ R6 / R8 / R10 / R11</em> 普通的通用寄存器</li><li><em>R7</em> 数据栈帧的指针，链接寄存器LR在栈上的地址</li><li><em>R9</em> 系统保留寄存器</li><li><em>R12</em> IP的值</li><li><em>R13</em> 栈顶指针SP</li><li><em>R14</em> 链接寄存器LR。所处代码作为函数的返回地址</li><li><em>R15</em> 程序计数器PC的值</li></ul><p><strong>说明：</strong> 其中，R14中存放的不是调用函数的地址，而是当前代码段执行完成后将跳转的地址</p><h3 id="判断即将跳转函数的参数个数" tabindex="-1"><a class="header-anchor" href="#判断即将跳转函数的参数个数" aria-hidden="true">#</a> 判断即将跳转函数的参数个数</h3><ul><li><code>BLX</code>的目标值是否为 <em>R0~R3</em> 寄存器（根据参数寄存器为 <em>R0~R3</em> 进行判断） <ul><li><p>是：参数个数小于4个时，<code>BLX</code>的跳转地址为目标函数地址，之前的寄存器皆为函数参数。例如：<code>BLX R3</code> 说明函数有3个参数，分别存储在 R0、R1、R2，而R3存放目标函数地址。</p></li><li><p>否：参数个数量大。前4个参数在 <em>R0~R3</em> 寄存器中。剩下的在跳转指令前通过<code>STR</code>或<code>STM</code>放入堆栈（并不一定入栈的数据都是参数，需要分析。若前面没有<code>STR</code>或<code>STM</code>压栈操作，则为4个）</p></li><li><p>分析：在被调用函数内，一开始便读取寄存器，一开始读取堆栈的，读取的数据为函数参数</p></li></ul></li></ul><p>可通过ida反编译的伪代码辅助分析。例如ida识别为方法名<code>BLX func_xxx</code>，通过 <em>tab</em> 查看ida解析出来的参数个数辅助自己分析。</p><div class="language-assembly line-numbers-mode" data-ext="assembly"><pre class="language-assembly"><code>mov r0, #5
mov r3, #4
mov r2, #3
str r0, [sp, #0]
mov r1, #2
mov r0, #1
BL func_test
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>此时，第5个参数在调用前被存储在堆栈上</p>`,32);function h(g,A){const n=a("ExternalLinkIcon");return r(),c("div",null,[o,e("p",null,[i("使用IDA工具并设置机器码的显示( Option -> General -> Disassembly -> Number of opcode bytes: 5)。以下两例均来自参考文章"),e("a",v,[i("https://bbs.kanxue.com/thread-268869.htm"),d(n)])]),m,e("ul",null,[e("li",null,[i("说明： "),e("ul",null,[u,e("li",null,[i("R3 加载内存偏移表对应项 "),b,i(" 的地址 "),R,i(" - "),D,e("a",p,[i("https://bbs.kanxue.com/thread-268869.htm"),d(n)])]),C])])]),_])}const x=s(t,[["render",h],["__file","011.html.vue"]]);export{x as default};
